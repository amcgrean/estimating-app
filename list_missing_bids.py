"""
List the 38 bids that safe_sync_bids_to_neon would insert, showing their local log_date.
"""
import sqlite3
...
def get_local_bids():
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT b.id, b.project_name, b.log_date, c.customerCode
        FROM bid b
...
def main():
    ...
    missing = []
    for l_id, l_proj, l_log, l_cc in local:
        cc = l_cc.strip().upper()
        best_score = 0
        best_match = None
        if cc in neon_by_cc:
        ...
        if not (best_match and best_score > 0.85):
            missing.append((l_id, l_proj, l_log, l_cc))

    print(f"Found {len(missing)} bids that would be inserted (log_date shown):")
    for bid in missing:
        print(f"ID {bid[0]:<5} | Log Date: {bid[2]} | Project: {bid[1]} | CustomerCode: {bid[3]}")
